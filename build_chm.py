...
class FileEntry:
    ...
    ascii_path: str     # CHM 内部路径 (如 f/00001.html)
...
def html_entity_encode(text: str) -> str:
    """
    将非 ASCII 字符转为 HTML 数字实体（&#NNNN; 格式）。
    这样文件本身为纯 ASCII，chmcmd 能正确解析，
    CHM 查看器渲染 HTML 时会自动将实体还原为中文。
    同时对 HTML 特殊字符（& < > "）进行转义。
    """
    result = []
    for ch in text:
...
def convert_all_to_html(source_dir: Path, build_dir: Path, files: dict) -> list:
    """
    将所有文件转为 HTML，使用纯 ASCII 文件名存储到 build_dir 中。
    返回 FileEntry 列表。
    """
    entries = []
    counter = 0
...
# ============================================================
# CHM 项目文件生成（核心修复：GBK 编码 + ASCII 路径）
# ============================================================
...
def generate_hhc(tree: dict, output_path: Path):
    """
    生成 .hhc 目录文件。
    显示名称用 GBK 编码（CHM 查看器按 Language=0x804 以 GBK 解码）。
    文件路径为纯 ASCII，不受编码影响。
    """
    lines = [
...
def main():
    ...
    # --- 2. 转换 ---
    print("[2/6] 转换文件为 HTML（使用 ASCII 文件名）...")

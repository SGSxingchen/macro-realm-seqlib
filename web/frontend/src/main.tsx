...
type AdminTab = 'overview' | 'edit' | 'package' | 'changes' | 'publish' | 'wiki';
...
type PackageFile = { path: string; filename: string; title: string; size: number; mtime: number; editable: boolean; extension: string; content?: string | null; encoding?: string | null };
...
function AdminPanel({ detail, reload, onResourceMoved, onBackToRead }: { detail: Detail | null; reload: () => void; onResourceMoved: () => void; onBackToRead: () => void }) {
  ...
  const tabs: Array<[AdminTab, string]> = [['overview','总览'],['edit','资源编辑'],['package','发行文件'],['changes','变更摘要'],['publish','发布版本'],['wiki','Wiki 同步']];
  ...
  return <section className="admin-console">
    ...
    <div className="admin-main terminal-scroll">
      ...
      {tab === 'package' && <div className="package-layout"><section className="console-card"><div className="section-head"><div><h3>ZIP 根目录发行文件</h3><p className="hint">这些是发行包根目录说明文件，不会出现在前台资源列表。</p></div><button onClick={() => runAction('刷新发行文件', loadPackageFiles)}>刷新列表</button></div><div className="package-list">{packageFiles.map(f => <button key={f.path} className={packageFile?.path === f.path ? 'package-item active' : 'package-item'} onClick={() => openPackageFile(f.path)}><b>{f.title}</b><small>{f.path} · {kb(f.size)} · {f.editable ? '可编辑' : '只读'}</small></button>)}</div></section><section className="console-card wide"><h3>编辑发行文件</h3>{packageFile ? <><div className="kv-grid"><span>文件</span><b>{packageFile.path}</b><span>状态</span><b>{packageFile.editable ? '根目录 TXT，可保存' : '只读展示'}</b></div><p className="hint">这是 ZIP/发行包根目录文件。保存会覆盖仓库根目录同名 .txt。</p><textarea className="editor terminal-scroll" disabled={!packageFile.editable} value={packageContent} onChange={e=>setPackageContent(e.target.value)} /><button disabled={!packageFile.editable} onClick={() => confirm(`确认覆盖根目录发行文件 ${packageFile.path}？`) && runAction('保存发行文件', savePackageFile)}>保存发行文件</button></> : <p className="hint">点击左侧文件开始编辑；非 .txt 文件只读展示。</p>}<div className="new-package-box"><h4>新增根目录 TXT</h4><label>文件名</label><input value={newPackageName} onChange={e=>setNewPackageName(e.target.value)} placeholder="例如 V6.5序列库更新日志.txt" /><button onClick={() => confirm(`确认新增根目录发行文件 ${newPackageName}？`) && runAction('新增发行文件', createPackageFile)}>以当前编辑区内容新增</button></div></section></div>}

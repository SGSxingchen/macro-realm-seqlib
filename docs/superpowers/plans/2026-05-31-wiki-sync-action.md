...
**Goal:** 新增一个独立的 GitHub Actions 手动工作流，用于 dry-run 或真实同步 Fandom Wiki。

**Architecture:** 工作流独立放在 `.github/workflows/wiki-sync.yml`，只通过 `workflow_dispatch` 触发。参数组装集中在 PowerShell 步骤里完成，真实同步通过 GitHub Secrets 读取凭据，dry-run 使用占位凭据。
